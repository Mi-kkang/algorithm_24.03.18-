...
def bfs(N, M):
    ...
    cnt1 = 0        # 익은 토마토 개수
    cnt2 = 0        # 빈칸 수
    visited = [[0]*M for _ in range(N)]     # visited 생성, 토마토가 익은 날짜
    for i in range(N) :         # 시작점 인큐 - 익은 토마토의 위치
        for j in range(M) :
            if tomato[i][j] == 1:   # 익은 토마토 == 1
                q.append((i,j))
    ...
    if cnt1 + cnt2 == N*M :  # 익은 토마토 수 + 빈칸 == 전체격차
        return 0
    ...
    # 가능한 토마토가 익었으면, 안익은 토마토 검사
    for i in range(N) :
        for j in range(M) :
            if tomato[i][j] == 0 and visited[i][j] == 0 :
                return -1       # 모두 익히는데 실패
    return last-1       # 마지막으로 익는데 걸리는 시간간
